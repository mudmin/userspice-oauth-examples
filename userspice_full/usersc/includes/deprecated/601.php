<?php
//the following functions were deprecated in v6.0.1. If you do not need these, you can delete them.
...
if (!function_exists('echoId')) {
  function echoId($id, $table, $column)
  {
    ...
    // Validate identifiers (letters/numbers/underscore, cannot start with digit)
    if (!is_string($table) || !is_string($column) ||
        !preg_match('/^[A-Za-z_][A-Za-z0-9_]*$/', $table) ||
        !preg_match('/^[A-Za-z_][A-Za-z0-9_]*$/', $column)) {
      error_log('echoId: Invalid table or column name rejected');
      return false;
    }

    $id = (int)$id;

    $query = $db->query("SELECT `$column` AS v FROM `$table` WHERE id = ? LIMIT 1", [$id]);
    if ($query->count() < 1) {
      echo 'Not in database';
      return false;
    }
    ...
    $value = null;
    if (is_object($row) && isset($row->v)) $value = $row->v;
    elseif (is_array($row) && array_key_exists('v', $row)) $value = $row['v'];
